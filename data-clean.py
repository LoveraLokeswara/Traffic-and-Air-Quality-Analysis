...
OUTPUT_FILE = PROJECT_ROOT / "cleaned_data_combined.csv"  # Final output file with all merged and cleaned data
...
def parse_stations(filepath: Path) -> list[dict]:
    ...
    ``weather:``) each followed by one or more non-blank lines.  Entries at
    the same *positional index* across sections are treated as belonging to
    the same logical station group.
...
def load_traffic(data_dir: Path, station_configs: list[dict]) -> pd.DataFrame:
    ...
    4. Aggregates traffic counts if multiple cameras map to the same station+date
    ...
    # Remove rows with invalid dates or traffic counts
    df_long = df_long.dropna(subset=["Date", "Traffic_Count"])

    # If multiple cameras/roads map to the same station on the same date, sum their counts
    # This consolidates multiple data sources into one traffic value per station-date
    df_final = df_long.groupby(["Station", "Date"], as_index=False)["Traffic_Count"].sum()
...
def main() -> pd.DataFrame:
    """Main pipeline: orchestrates all cleaning steps in sequence.
    
    COMPLETE WORKFLOW:
    1. Parse configuration file (stations.txt) to link data sources
    2. Load and clean Air Quality data (hourly → daily, remove sentinel values)
    3. Load and clean Traffic data (wide → long format, aggregate by station)
    4. Load and clean Weather data (normalize units and column names)
    5. Merge all three datasets on (Station, Date)
    5.5. Filter data to specified timeframe (2022/02/02 – 2024/12/31)
    6. Report missing values before imputation
    7. Perform multiple imputation on missing values
    8. Report missing values after imputation
    9. Save cleaned, complete dataset to CSV
    """
    print("=" * 60)
    ...
    print("\n[2/6] Loading Air Quality data ...")
    df_aq = load_air_quality(DATA_DIR, station_configs)

    print("\n[3/6] Loading Traffic data ...")
    ...
    # ---- Step 5.5: Filter by timeframe ----
    # Restrict data to the specified date range for consistent analysis window
    print("\n[5.5/6] Filtering data to specified timeframe ...")
    df_merged = filter_data_by_date(df_merged, START_DATE, END_DATE)

    # ---- Report before imputation ----
    # Analyze how much missing data exists before filling in estimates
    missing_before = report_missing(df_merged, "(Before Imputation)")

    # ---- Step 6: Multiple Imputation ----
    # Uses MICE algorithm to estimate missing values based on relationships
    # between variables, then applies domain-specific constraints
    print("\n[6/6] Performing Multiple Imputation (IterativeImputer) ...")
    df_final = impute_data(df_merged)

    # ---- Report after imputation ----
    # Verify that imputation successfully filled missing values
    missing_after = report_missing(df_final, "(After Imputation)")

    # ---- Save output ----
    # Write the cleaned, complete dataset to CSV for downstream analysis
    df_final.to_csv(OUTPUT_FILE, index=False)
    print(f"\nSaved cleaned data to: {OUTPUT_FILE}")
    print(f"Final shape: {df_final.shape[0]} rows x {df_final.shape[1]} columns")

    # ---- Comparison summary ----
    # Show impact of imputation: how many values were filled for each column
    print(f"\n{'='*60}")
    print("  Imputation Summary")
    print(f"{'='*60}")
    numeric_cols = df_merged.select_dtypes(include=[np.number]).columns
    for col in numeric_cols:
        before = int(missing_before.get(col, 0))  # Missing values before imputation
        after = int(missing_after.get(col, 0))  # Missing values after imputation
        if before > 0:
            print(f"  {col:20s}  {before:>6d} -> {after:>6d} missing  (filled {before - after})")
