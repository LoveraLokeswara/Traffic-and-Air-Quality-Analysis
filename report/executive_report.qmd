---
title: "Traffic and Air Quality in Toronto:"
subtitle: "Implications for TTC Strategy"
author:
  - Dario Greco
  - Lovera Lokeswara
  - Sophia Feng
...
This study integrates traffic volume, air quality (NO$_2$​), and meteorological data  from February 2022 to December 2024 across five monitoring stations across Ontario to help evaluate the relationship between road traffic and urban air pollution in Toronto. Following data cleaning, imputation, and feature engineering, both exploratory analyses and predictive modeling approaches including XGBoost, Random Forest, Generalized Additive Models (GAM), and a stacking ensemble were implemented using time-series cross-validation. Model comparisons assessed performance with and without traffic-related features to determine their incremental predictive value. Results indicate that traffic has a relatively weak relationship with NO$_2$, while seasonal and meteorological factors have a more dominant impact, and nonlinear models provide better predictive performance in capturing these complex relationships.
...
A major limitation of this analysis was the spatial discrepancy between the data measurement stations. For instance, the meteorological monitoring station used for Toronto West was approximately 7.68 km away from the air quality station, while the traffic station was roughly a kilometer away from the air quality station. 
A similar spatial mismatch affected the other locations, introducing potential bias when inferring a relationship between traffic at Point A and air quality at Point B.
...
![Training vs Testing (with Traffic)](../results/cv_vs_test_with_traffic.png){#fig-test-vs-train-with-traffic fig-pos='H'}

![Training vs Testing (without Traffic)](../results/cv_vs_test_no_traffic.png){#fig-test-vs-train-no-traffic fig-pos='H'}
...
![Residual Distribution (with Traffic)](../results/residuals_with_traffic.png){#fig-residual-distribution-with-traffic fig-pos='H'}

![Residual Distribution (without Traffic)](../results/residuals_no_traffic.png){#fig-residual-distribution-no-traffic fig-pos='H'}

```{=latex}
\newpage
```

## Appendix D — Station Distance Summary

Table: Distance from AQ station to nearest traffic and weather stations {#tbl-station-distance}

| Location           | Distance to Traffic Station | Distance to Weather Station |
|--------------------|-----------------------------|-----------------------------|
| Toronto Downtown   | 0.18 km                     | 1.93 km                     |
| Toronto West       | 0.83 km                     | 7.69 km                     |
| Ottawa             | 1.95 km                     | 6.95 km                     |
| Sarnia             | 0.38 km                     | 7.02 km                     |
| Sault Ste. Marie   | 6.20 km                     | 17.13 km                    |
